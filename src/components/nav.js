...
export function renderNav(currentPage) {
  ...
  nav.innerHTML = `
    ...
    <nav class="nav-links" id="nav-links" role="navigation" aria-label="Main navigation">
      ...
      <button class="nav-cta ${currentPage === 'submit' || currentPage === 'submit-entry' || currentPage === 'team-registration' ? 'nav-cta-active' : ''}" data-page="submit" id="nav-btn-submit" aria-label="Submit Form">
        <span class="cta-dot"></span> SUBMIT FORM
      </button>
    </nav>
  `
